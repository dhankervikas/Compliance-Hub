...
const Dashboard = () => {
    ...
    const { tenantId } = useParams(); // Get Tenant Context
    const baseUrl = tenantId ? `/t/${tenantId}` : '';
    const { entitlements, loading: loadingEntitlements } = useEntitlements(); // Use Entitlement Context
    ...
    useEffect(() => {
        console.log("DEBUG: Dashboard Entitlements:", entitlements); // DEBUG LOG
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Add Tenant Context Header if we are viewing a specific tenant
            if (tenantId) {
                headers['X-Target-Tenant-ID'] = tenantId; // Use masquerading header
            }
            ...
            // 1. Filter Frameworks
            // If viewing a specific tenant, fetch list from API to ensure we get Their IDs (not Admin's)
            let activeFrameworks = [];
            if (tenantId) {
                try {
                    const listRes = await axios.get(`${API_URL}/frameworks/`, { headers });
                    activeFrameworks = listRes.data.filter(fw => fw.is_active);
                } catch (err) {
                    console.error("Failed to fetch tenant frameworks list", err);
                    activeFrameworks = [];
                }
            } else {
                // Standard User: Use Entitlements source of truth
                activeFrameworks = entitlements.frameworks.filter(fw => fw.is_active);
            }
            ...
            const frameworksWithStats = await Promise.all(activeFrameworks.map(async (fw) => {
                try {
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            const DEFAULT_FRAMEWORKS = [
                ...
                { name: "ISO 42001:2023", code: "ISO42001", description: "Artificial Intelligence Management System" }
            ];
            ...
            addLog("Frameworks Validated.");
            // Client-side control seeding is disabled to prevent overwriting backend Single Source of Truth.
            // Use backend scripts (e.g., regenerate_all_iso_text.py) for data repairs.

            addLog("Global Seed Complete.");
            alert("Database frameworks verified. For control data, please use backend repair scripts.");
            setLoading(false);
            fetchData();
            addLog("Global Seed Complete.");
