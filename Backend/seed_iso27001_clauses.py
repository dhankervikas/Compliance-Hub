"""
Seed script to populate ISO 27001:2022 ISMS Requirements (Clauses 4-10)
Run this script to populate your database with mandatory ISMS requirements
...
def create_framework(db: Session):
    """Create ISO 27001:2022 ISMS Requirements framework"""
    # Check if framework already exists
    existing = db.query(Framework).filter(Framework.code == "ISO27001_ISMS").first()
    if existing:
        print(f"Framework already exists: {existing.name} (ID: {existing.id})")
        return existing
    
    framework = Framework(
        name="ISO 27001:2022 - ISMS Requirements",
        code="ISO27001_ISMS",
        description="Information Security Management System Requirements - Mandatory requirements from Clauses 4-10",
        version="2022"
    )
    db.add(framework)
    db.commit()
...
def create_requirements(db: Session, framework_id: int):
    """Create all ISO 27001:2022 ISMS requirements from Clauses 4-10"""
    ...
    existing_count = db.query(Control).filter(Control.framework_id == framework_id).count()
    if existing_count > 0:
        print(f"Found {existing_count} existing requirements for this framework")
        response = input("Do you want to skip existing requirements? (y/n): ")
        if response.lower() != 'y':
            print("Aborted. Delete existing requirements first if you want to re-seed.")
            return
    ...
    for requirement_data in requirements_data:
        ...
        if existing:
            print(f"Skipping existing requirement: {requirement_data['control_id']}")
            skipped_count += 1
            continue
        ...
        db.add(requirement)
        created_count += 1
        print(f"Created requirement: {requirement_data['control_id']} - {requirement_data['title']}")
    
    db.commit()
    print(f"\n{'='*80}")
    print(f"Successfully created {created_count} requirements")
    if skipped_count > 0:
        print(f"Skipped {skipped_count} existing requirements")
    print(f"{'='*80}\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("ISO 27001:2022 ISMS Requirements (Clauses 4-10) Seeding Script")
    print("="*80 + "\n")
    ...
    try:
        ...
        print("Step 1: Creating ISO 27001:2022 ISMS Requirements framework...")
        framework = create_framework(db)
        print()
        
        # Step 2: Create requirements
        print("Step 2: Creating all ISMS requirements from Clauses 4-10...")
        create_requirements(db, framework.id)
        
        print("\n" + "="*80)
        print("ISO 27001:2022 ISMS Requirements seeding completed successfully!")
        print("="*80 + "\n")
        
        # Summary
        total_requirements = db.query(Control).filter(Control.framework_id == framework.id).count()
        print(f"Summary:")
        print(f"   Framework: {framework.name}")
        print(f"   Total Requirements: {total_requirements}")
        print(f"   Clauses:")
        print(f"     - Clause 4: Context of the organization (4 requirements)")
        print(f"     - Clause 5: Leadership (3 requirements)")
        print(f"     - Clause 6: Planning (5 requirements)")
        print(f"     - Clause 7: Support (7 requirements)")
        print(f"     - Clause 8: Operation (3 requirements)")
        print(f"     - Clause 9: Performance evaluation (3 requirements)")
        print(f"     - Clause 10: Improvement (2 requirements)")
        print()
        print(f"These are MANDATORY requirements for ISO 27001:2022 certification")
        print(f"All requirements must be addressed for compliance")
        print()
