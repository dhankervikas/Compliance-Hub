...
import {
  ChevronDown, Plus, Edit2, CheckCircle, FileText, AlertCircle, MoreHorizontal, Search
} from 'lucide-react';
    ...
    try {
      ...
      const filteredProcesses = procRes.data.map(proc => {
        const relevantSubs = proc.sub_processes.map(sub => {
          // Simplistic matching: if sub-process controls exists, filter for current framework
          let mappedControls = sub.controls ? sub.controls.filter(c => c.framework_id === parseInt(id)) : [];

          // SORTING FIX: Ensure controls are sorted by Control ID (e.g. 5.1, 5.2, A.5.1)
          mappedControls.sort((a, b) => {
            // Custom sort to handle "A." prefix vs numeric
            // Normalize: "A.5.1" -> "A005.001", "5.1" -> "005.001"
            const normalize = (cid) => {
              let s = cid.toUpperCase();
              let isAnnex = s.startsWith('A');
              if (isAnnex) s = s.substring(2); // Remove A. or A

              // Split by dots
              const parts = s.split('.').map(p => p.padStart(3, '0'));
              return (isAnnex ? 'Z' : 'A') + parts.join(''); // Force Annex (Z) after Clauses (A)
            }
            return normalize(a.control_id).localeCompare(normalize(b.control_id));
          });

          return { ...sub, controls: mappedControls };
      ...
      // If the process view is empty (maybe due to mapping issues), fallback to raw framework stats
      const fwControls = allControls.filter(c => c.framework_id === parseInt(id));
      total = fwControls.length;
      implemented = fwControls.filter(c => c.status === 'IMPLEMENTED').length;
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="space-y-10">
          {processes.length === 0 ? <div className="text-center py-10 text-gray-500">No mapped processes found for this framework. <button className="text-blue-600 underline" onClick={() => window.location.reload()}>Refresh</button></div> : processes.map(process => (
            <div key={process.id}>
