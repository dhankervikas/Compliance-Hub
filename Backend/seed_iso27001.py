"""
Seed script to populate ISO 27001:2022 framework and all 93 controls
Run this script once to populate your database with ISO 27001:2022 data
...
def create_controls(db: Session, framework_id: int):
    """Create all ISO 27001:2022 controls"""
    ...
    if existing_count > 0:
        print(f"Found {existing_count} existing controls for this framework")
        response = input("Do you want to skip existing controls? (y/n): ")
        if response.lower() != 'y':
            print("Aborted. Delete existing controls first if you want to re-seed.")
            return
    ...
    for control_data in controls_data:
        ...
        if existing:
            print(f"Skipping existing control: {control_data['control_id']}")
            skipped_count += 1
            continue
    ...
    db.commit()
    print(f"\n{'='*80}")
    print(f"Successfully created {created_count} controls")
    if skipped_count > 0:
        print(f"Skipped {skipped_count} existing controls")
    print(f"{'='*80}\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("ISO 27001:2022 Database Seeding Script")
    print("="*80 + "\n")
    ...
    try:
        ...
        print("Step 1: Creating ISO 27001:2022 framework...")
        framework = create_framework(db)
        print()
        
        # Step 2: Create controls
        print("Step 2: Creating all 93 ISO 27001:2022 controls...")
        create_controls(db, framework.id)
        
        print("\n" + "="*80)
        print("ISO 27001:2022 seeding completed successfully!")
        print("="*80 + "\n")
        
        # Summary
        total_controls = db.query(Control).filter(Control.framework_id == framework.id).count()
        print(f"Summary:")
        print(f"   Framework: {framework.name}")
        print(f"   Total Controls: {total_controls}")
        print(f"   Categories:")
        print(f"     - Organizational controls: 37")
        print(f"     - People controls: 8")
        print(f"     - Physical controls: 14")
        print(f"     - Technological controls: 34")
        print()
